...
class SchoolAccessMiddleware:
    ...
    def __call__(self, request):
        if request.user.is_authenticated:
            ...
            if request.user.user_type == 1 and request.user.school and not request.user.school.is_active:
                logger.debug(f"Logging out admin {request.user.username} due to inactive school")
                logout(request)
